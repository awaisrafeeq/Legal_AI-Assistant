...
class Config:
    ...
    search_key: str
    search_index: str
    # FIX: two separate indices — external and internal never share a search call
    search_index_external: str
    search_index_internal: str
    # FIX: below this score both sources return "not found" — no GPT-4 call, no hallucination
...
class ChatRequest(BaseModel):
    ...
    history: Optional[List[ChatMessage]] = []   # NEW: conversation history
    source_mode: Optional[str] = "all"   # all / external_only / internal_only ### Testing
    allowed_files: Optional[List[str]] = [] ### Testing
...
def detect_language(text: str, llm: AzureChatOpenAI) -> str:
    ...
    """
    # Fast path: detect Urdu/Arabic by Unicode range (no LLM call needed)
    ...
    # Fast path: detect French by common function words
...
Language code:"""
    ...
    # Return first valid 2-letter code found
    import re
...
class AzureClients:
    def __init__(self, config: Config):
        ...
        # FIX: two separate SearchClients — one per index
        # External and internal data never mix in a single search call
        ...
        # IMPROVEMENT: temperature 0.1 for factual, consistent legal answers
...
class RAGState(TypedDict, total=False):
    ...
    winning_source: str           # FIX: which index won — "legal-documents" or "legal-documents-internal"
    context: str
    answer: str
    sources: List[Dict]
    conversation_history: List[Dict]
    source_mode: str      ### Testing
    allowed_files: List[str] ### Testing
    lookup_mode: str   ### Testing
...
#     Routing logic:
#     - Internal docs (legal-documents-internal): root level of internal container
#     - External docs (legal-documents): folder_path/file_name inside external container
#     """
#     try:
#         from urllib.parse import urlparse, quote

#         # Get original PDF name — strip .txt suffix added by OCR pipeline
#         original_name = (file_name or blob_path.split("/")[-1])
...
#             # Internal: PDF sits at root of legal-documents-internal container
#             sas_url = azure_clients.config.internal_container_sas_url
#             blob_file_path = original_name
#         else:
#             # External: PDF sits inside folder structure of legal-documents container
#             sas_url = azure_clients.config.container_sas_url
#             blob_file_path = f"{folder_path}/{original_name}" if folder_path else original_name
...
def generate_sas_url(
    ...
    try:
        ...
        if is_internal and azure_clients.config.internal_container_sas_url:
            ...
            # Convert OCR text blob path back to original internal PDF path
            ...
            # EXTERNAL: convert OCR/indexed path back to original PDF path
            sas_url = azure_clients.config.container_sas_url

            if folder_path and file_name:
                # folder_path + file_name are reliably set by the indexer from the
                # OCR blob path — use them directly to reconstruct the original PDF path.
                blob_file_path = f"{folder_path}/{file_name}"
            elif blob_path:
                # Fallback: strip extracted-text/ prefix and .txt suffix from blob_path
...
def _search_one_index(
    ...
    query_embedding: List[float],
    top_k: int,
    allowed_files: Optional[List[str]] = None ### Testing
) -> List[Dict]:
    """
    ...
    #         # Semantic reranker score (0-4 scale) normalized to 0-1.
    #         # Falls back to hybrid BM25+vector score if reranker not available.
    ...
    #     }
    #     for r in results
    # ]   ###### commented for below code test, apply hard filter for files 
    results = search_client.search(
    search_text=search_query,
    vector_queries=[vector_query],
    query_type="semantic",
    ...
    # HARD FILTER — enforce allowed_files after search results come back
    if allowed_files:
...
def hybrid_search_isolated(
    ...
    if source_mode in ("all", "external_only"):
        external_results = _search_one_index(
            ...
            query_embedding, 
            top_k,
            allowed_files=list(allowed_files) if allowed_files else None ### Testing
        )
    ...
    #         if r.get("file_name") in allowed_files
    #     ]
    #     internal_results = [
    ...
    # Both below threshold → refuse to answer, no GPT-4 call
    if ext_best < MIN_SCORE and int_best < MIN_SCORE:
        logger.warning("Both sources below confidence threshold — returning no results")
        return [], "none"


    ################### commented for not mixing internal and external chunks #############################
    # MERGED ARCHITECTURE — both indexes contribute chunks.
    # Strategy: guarantee minimum slots from each source so that relevant
    # external documents (e.g. SQ investigation, Air Canada records) always
    # reach GPT-4 even when internal chunks score slightly higher overall.
    #
    # Slot allocation (top_k=15 default):
    #   - 5 best from internal (if available above threshold)
    #   - 5 best from external (if available above threshold)
    #   - remaining 5 filled by highest scoring chunks from either source
    ...
    # GUARANTEED = top_k // 3  # 5 slots guaranteed per source when top_k=15

    # guaranteed_int = filtered_internal[:GUARANTEED]
    # guaranteed_ext = filtered_external[:GUARANTEED]

    # # Remaining budget filled by best of whatever is left from either source
    # rest = filtered_internal[GUARANTEED:] + filtered_external[GUARANTEED:]
    # rest.sort(key=lambda r: r["score"], reverse=True)
    # remaining_budget = top_k - len(guaranteed_int) - len(guaranteed_ext)

    # winning = guaranteed_int + guaranteed_ext + rest[:max(0, remaining_budget)]

    # # Re-sort by score so the most relevant chunk is first regardless of source.
    # # This ensures WhatsApp bot's top-3 display shows the most relevant sources,
    # # not just the first guaranteed internal slots.
    # winning.sort(key=lambda r: r["score"], reverse=True)

    # # Label winning source for logging / response metadata
    ...
    # # Generate correct SAS URLs for each chunk based on its own container
    # for r in winning:
    ...
    # logger.info(
    #     f"Merged: {len(winning)} chunks | "
    #     f"Internal: {len(filtered_internal)} | External: {len(filtered_external)} | "
    #     f"Sources: {sources_in_winning} | Best score: {winning[0]['score']:.4f}"
    # )
    ...
    # Choose ONE winning source only
    ...
    ###################### removing 1 doc winning logic #####################################

    # Group by file_name and choose ONE winning file only
    ...
    # def file_score(items):
    #     items = sorted(items, key=lambda x: x["score"], reverse=True)
    #     top = items[0]["score"]
    #     bonus = sum(x["score"] for x in items[:3]) * 0.1
    #     return top + bonus

    # winning_file, winning_file_chunks = max(
    #     by_file.items(),
    #     key=lambda kv: file_score(kv[1])
    # )

    # winning = sorted(winning_file_chunks, key=lambda x: x["score"], reverse=True)[:5]
    ...
    # logger.info(
    #     f"Single-doc mode: {len(winning)} chunks | "
    #     f"Source: {winning_source} | File: {winning_file} | "
...
def rewrite_query_node(state: RAGState, azure_clients: AzureClients) -> RAGState:
    ...
    # Detect original language BEFORE rewriting (rewritten query is always French)
    ...
    #################### test block #########################
    # Build history context for the rewriter
    history_text = ""
    if history:
        last_turns = history[-4:]  # Last 2 turns max
    ...
    prompt = f"""You are a search query optimizer for a French-language legal document database.
...
French keyword search query:"""
    ...
    return {
        ...
        "conversation_history": history,
        "lookup_mode": lookup_mode,                         ### Testing
        "source_mode": state.get("source_mode", "all"),     ### Testing
        "allowed_files": state.get("allowed_files", [])     ### Testing
    }
...
def retrieve_node(state: RAGState, azure_clients: AzureClients) -> RAGState:
    ...
    # FIX: use isolated search — results come from ONE index only, never mixed
    search_results, winning_source = hybrid_search_isolated(
        azure_clients,
        search_query, 
        query_embedding, 
        top_k=15,
        source_mode=source_mode, ### Testing
        allowed_files=allowed_files ### Testing
    )
...
def generate_node(state: RAGState, azure_clients: AzureClients) -> RAGState:
    ...
    lookup_mode = state.get("lookup_mode", "answer") ### test
    

    # FIX: zero-results guard — both indices were below confidence threshold
    # Do NOT call GPT-4. Return a definitive "not found" — no hallucination possible.
    ...
    if lookup_mode == "document":
        ...
        for s in search_results:
            ...
            # If user explicitly wants original PDFs, avoid transcript/interrogation files
        ...
        # Enforce single-document context only   #### this is statement is for testing jis sy aik hi file ka chunk aye ga ### removing sigle doc logic
    # if search_results:
    #     top_file = search_results[0]["file_name"]
    #     search_results = [r for r in search_results if r["file_name"] == top_file][:5]
    
    # FIX: source_type check uses exact value "legal-documents-internal"
    # (was checking correctly but now aligns with what index_internal.py actually writes)
    context_parts = []
    ...
    # Determine response language from detected query language
    ...
    # IMPROVEMENT: Completely rewritten system prompt
    system_prompt = f"""You are an expert legal AI assistant for a French-language legal document system.
...
async def search_documents(request: SearchRequest):
    ...
    try:
        ...
        # FIX: use isolated search — no data mixing
...
async def chat(request: ChatRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Step 3: Run RAG (search in original language — no unnecessary translation)
        # Documents are bilingual so searching in original language works better
        ...
        state: RAGState = {
            ...
            "source_mode": request.source_mode or "all",  ### Testing
            "allowed_files": request.allowed_files or [] ### testing
        }
        ...
        # Email intent — send answer in background if user requested it
